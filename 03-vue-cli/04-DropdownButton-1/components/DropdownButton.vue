<template>
  <div class="dropdown" :class="{ 'show': open }" @click="toggleDropdown">
    <button
      type="button"
      class="button dropdown__toggle"
      :class="{ 'dropdown__toggle_icon': isIconSupported }"
    >
      <app-icon v-if="selected && selected.icon" :icon="selected.icon" />
      {{ value ? `${title} - ${selected.text}` : title }}
    </button>

    <div class="dropdown__menu" :class="{ 'show': open }">
      <button
        v-for="option in options"
        :key="option.value"
        class="dropdown__item"
        :class="{ 'dropdown__item_icon': isIconSupported }"
